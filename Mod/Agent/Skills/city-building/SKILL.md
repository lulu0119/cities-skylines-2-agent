...
- For infrastructure and service buildings without a player-selected site, call find_infrastructure_candidate(role) first. It chooses an unlocked typed prefab and returns one native-validated position satisfying that prefab's declared shoreline and road requirements. Commit that exact prefab/position/rotation with place_building. When the site or prefab is already known, place_building with x/z/radius remains the one-step search-and-place path. Utility placement uses prefab node flags to reuse road-carried networks or add the appropriate off-road pipe/cable.

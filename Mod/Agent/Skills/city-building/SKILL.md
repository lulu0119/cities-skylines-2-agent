...
- For infrastructure and service buildings without a player-selected site, call find_infrastructure_candidate(role) first. It chooses an unlocked typed prefab and returns one native-validated, road-facing position with shoreline evidence when required. Commit that exact prefab/position/rotation with place_building. When the site or prefab is already known, place_building with x/z/radius remains the one-step search-and-place path. Utility placement handles the required connection automatically: road-fronted water pumps use the pipes carried by the road, while sewage outlets and power producers receive the appropriate short pipe/cable.
...
- Empty land: connect a road from the highway, then use find_infrastructure_candidate for power, water and sewage sites. Use terrain, gridmap and find_prefabs only when inspecting alternatives or resource context.
